...
export function renderMonitor(data, view, ui) {
  ...
  const graph = `<section class="card"><div class="cardhead"><h2>Bản đồ hoạt động</h2><span class="badge gray">${data.totals.running} đang xử lý</span></div><div class="cardpad"><div class="monitor-map"><svg class="monitor-wires" aria-hidden="true"></svg><div class="monitor-mapcol"><h3>AGENT</h3>${nodes('agents', 'actor')}</div><div class="monitor-mapcol"><h3>KẾT NỐI MCP</h3>${nodes('connections', 'mcp')}</div></div><div class="monitor-mapfoot">Đường nối thể hiện lượt gọi trong khoảng chọn. Bấm agent hoặc kết nối để xem riêng.</div><details class="monitor-relations"><summary>Quan hệ cấp quyền và sử dụng (${relations.length})</summary>${relations.map(r => `<div class="listrow"><span>${esc(name('agents', r.agentId))} → ${esc(name('connections', r.mcpId))}</span><span>${r.granted} quyền đã cấp · ${r.callable || 0} dùng được · ${number(r.calls)} lượt</span></div>`).join('')}</details></div></section>`;
