...
`GET /api/logs` và tool admin `audit_list` nhận bộ lọc tùy chọn `actor` (ID hoặc tên hiển thị agent), `connector`/`mcp`, `tool`, `status` (`ok`/`success`/`error`/`denied`), `since`/`until` (ISO 8601), `minLatency`/`maxLatency` (ms), `id`, `secret`, `cursor` (phân trang keyset `id < cursor`), `paginate` (`true`/`false`), `includePayload` (`true`/`false`), `limit` (1–5.000). Các bộ lọc kết hợp bằng AND và thực thi ở tầng cơ sở dữ liệu SQLite trước `LIMIT`. Với phân trang và danh sách, API trả về `{ rows, nextCursor, hasMore }` kèm metadata; chi tiết đầy đủ của từng bản ghi được tải riêng qua `GET /api/logs/:id` để tránh giải mã payload hàng loạt. Response tiếp tục yêu cầu phiên owner; tool admin yêu cầu token quản trị riêng.
