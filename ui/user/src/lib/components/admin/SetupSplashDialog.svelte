		...
		<div class="bg-slate-950 text-white p-6 relative border-b border-slate-800">
			<div class="flex items-center justify-between">
				<div class="flex items-center gap-3">
					...
					<span
						class="text-xs px-2.5 py-1 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-400/30 font-bold uppercase tracking-wider"
					>
						Quick Setup Wizard
					</span>
				</div>
			...
			<p class="text-xs text-slate-300 mt-1">
				Hoàn tất 4 bước đơn giản để kết nối Agent của bạn với Gen Hub.
			</p>

			<!-- 4-Step Progress Indicator -->
			<div class="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-5">
				{#each [{ step: 1, label: '1. Tên miền & Mạng', icon: Globe }, { step: 2, label: '2. Scope Agent', icon: KeyRound }, { step: 3, label: '3. MCP Catalog', icon: RadioTower }, { step: 4, label: '4. Két bảo mật', icon: ShieldCheck }] as item (item.step)}
					<button
						type="button"
		...
		<div class="p-6 flex flex-col min-h-[340px] justify-between bg-white dark:bg-slate-900">
				...
				<div class="flex flex-col gap-4" in:fade={{ duration: 150 }}>
					<div>
						...
						<p class="text-xs text-slate-600 dark:text-slate-400 mt-1">
							Gen Hub hỗ trợ mở cổng truy cập linh hoạt từ Localhost, Tên miền riêng hoặc Cloudflare
							Tunnel.
						</p>
					</div>
					...
					<div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-2">
						...
						>
							<div class="flex items-center justify-between">
								<span class="text-xs font-bold text-indigo-600 dark:text-indigo-400"
									>Cục bộ (Local)</span
								>
								{#if domainMode === 'localhost'}<CheckCircle2
							...
							<div>
								...
								<div class="text-[11px] text-slate-600 dark:text-slate-400 mt-0.5 font-medium">
									Khởi chạy nhanh trên máy cá nhân
								</div>
							</div>
						</button>
						...
						>
							<div class="flex items-center justify-between">
								...
								>
								{#if domainMode === 'tunnel'}<CheckCircle2
										class="size-4 text-indigo-600 dark:text-indigo-400"
									/>{/if}
							</div>
							<div>
								...
								<div class="text-[11px] text-slate-600 dark:text-slate-400 mt-0.5 font-medium">
									Mở cổng an toàn ra Internet không cần NAT Port
								</div>
							</div>
						</button>
						...
						>
							<div class="flex items-center justify-between">
								<span class="text-xs font-bold text-indigo-600 dark:text-indigo-400"
									>Tên miền riêng</span
								>
								{#if domainMode === 'domain'}<CheckCircle2
										class="size-4 text-indigo-600 dark:text-indigo-400"
									/>{/if}
							</div>
							<div>
								...
								<div class="text-[11px] text-slate-600 dark:text-slate-400 mt-0.5 font-medium">
									Gắn SSL & Caddy/Nginx reverse proxy
								</div>
							</div>
						</button>
					</div>
					...
					<div
						class="p-3.5 rounded-xl bg-slate-950 text-slate-100 text-xs font-mono flex items-center justify-between border border-slate-800"
					>
						<div class="flex items-center gap-2">
							<Terminal class="size-4 text-indigo-400 shrink-0" />
							<span class="text-slate-200"
								>Lệnh khởi động Server: <strong class="text-emerald-400 font-bold"
									>./bin/gen-hub server</strong
								></span
							>
						</div>
						<span
							class="text-[10px] text-slate-300 px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-sans font-bold"
							>Cổng: 8080</span
						>
					</div>
				</div>
			{:else if currentStep === 2}
				...
				<div class="flex flex-col gap-4" in:fade={{ duration: 150 }}>
					<div>
						...
						<p class="text-xs text-slate-600 dark:text-slate-400 mt-1">
							Tạo một API Key gắn liền với chính sách truy cập MCP Server và LLM Proxy cho Agent của
							bạn (Claude Code, Cursor, Windsurf...).
						</p>
					</div>

					{#if !createdApiKey}
						<div
							class="flex flex-col gap-3 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50"
						>
							<label
								for="wizard-scope-name-input"
							...
							<p class="text-[11px] text-slate-600 dark:text-slate-400">
								Scope này tự động cấp quyền truy cập các MCP tool và LLM proxy cần thiết.
							</p>
						</div>
					{:else}
						<div
							class="flex flex-col gap-3 p-4 rounded-2xl border border-emerald-500/30 bg-emerald-50/50 dark:bg-emerald-950/20"
						>
							<div
								class="flex items-center gap-2 text-emerald-700 dark:text-emerald-400 font-bold text-xs"
							>
							...
							<p class="text-[11px] text-slate-600 dark:text-slate-400">
								Vui lòng sao chép API Key này để dán vào cấu hình Agent của bạn.
							</p>
						</div>
					{/if}
				</div>
			{:else if currentStep === 3}
				...
				<div class="flex flex-col gap-4" in:fade={{ duration: 150 }}>
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
						{#each [{ key: 'webSearch', name: 'Web Search Tool', desc: 'Cho phép Agent tìm kiếm Google/Brave trên Web' }, { key: 'gdrive', name: 'Google Drive MCP', desc: 'Đọc và duyệt tài liệu Google Docs / Sheets' }, { key: 'github', name: 'GitHub Integration', desc: 'Truy vấn repository, commit và PR' }, { key: 'filesystem', name: 'Local Filesystem', desc: 'Thao tác đọc ghi tệp tin trên máy cục bộ' }] as tool (tool.key)}
							...
							>
								...
								<div class="flex flex-col min-w-0">
									...
									<div
										class="text-[11px] text-slate-600 dark:text-slate-400 mt-0.5 line-clamp-1 font-medium"
									>
										{tool.desc}
									</div>
								</div>
			...
			{:else if currentStep === 4}
				...
				<div class="flex flex-col gap-4 text-center items-center py-2" in:fade={{ duration: 150 }}>
					<div
						class="size-16 rounded-full bg-emerald-100 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 flex items-center justify-center"
					>
					...
					<p
						class="text-xs text-slate-600 dark:text-slate-300 max-w-lg leading-relaxed font-medium"
					>
						Hệ thống Mã hóa Master Vault (AES-256 GCM) đã kích hoạt. Mọi OAuth credentials, API keys
						và thông tin truy cập đều được lưu giữ bảo mật tuyệt đối tại máy của bạn.
					</p>

					<div
						class="flex items-center gap-3 p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-xs text-slate-800 dark:text-slate-200"
					>
						<Sparkles class="size-4 text-amber-500 shrink-0" />
						<span
							>Bạn có thể tùy chỉnh lại cài đặt bất kỳ lúc nào từ thanh điều hướng <strong
								>Domain & Cài đặt</strong
							>.</span
						>
					</div>
				</div>
			...
			<div
				class="flex items-center justify-between border-t border-slate-200 dark:border-slate-800 pt-4 mt-4"
			>
				<button
					type="button"
