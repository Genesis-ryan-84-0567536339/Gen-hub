...
<div class="bg-base-200 text-base-content flex h-full w-[inherit] min-w-[inherit] flex-col">
	<!-- Inspector Header -->
	<div
		class="dark:bg-base-300 bg-base-100 relative flex w-full flex-col p-4 pl-5 shadow-xs border-b border-base-300 dark:border-base-400"
	>
		<div
			class={twMerge(
		...
		<div class="flex items-center justify-between pr-10">
			<div class="flex items-center gap-2">
				<span class="text-xs font-bold uppercase tracking-wider text-primary"
					>Request Inspector</span
				>
				<span class="text-xs text-muted-content font-mono"
					>#{auditLog.id?.toString().slice(0, 10)}</span
				>
			</div>
		...
		<h3
			class="text-lg font-bold tracking-tight mt-1 text-slate-900 dark:text-white flex items-center gap-2"
		>
	...
	<div class="default-scrollbar-thin relative h-[calc(100%-72px)] overflow-y-auto pb-6">
		<!-- Summary Chips -->
		<div
			class="flex flex-wrap gap-2 py-4 px-5 border-b border-base-300 dark:border-base-400/50 bg-base-100/50 dark:bg-base-200/50"
		>
			{@render chip(
				'Trạng thái',
				auditLog.outcome.status,
				auditLog.outcome.status === 'success'
		...
		<div class="px-5 flex flex-col gap-6 pt-5">
			...
			<div class="flex flex-col gap-2">
				<div class="flex items-center justify-between">
					...
					{#if details && !details.payloadRedacted && hasBody(details.request?.body)}
						<CopyButton
							text={typeof details.request?.body === 'string'
				...
				{:else if details && hasBody(details.request?.body)}
					<div
						class="relative rounded-xl overflow-hidden border border-base-300 dark:border-base-400"
					>
						<JsonPreview
							value={details.request?.body}
							ariaLabel="Request Input Arguments"
							maximizable
						/>
					</div>
				{:else}
					<div
						class="bg-base-100 dark:bg-base-300/40 text-muted-content rounded-xl p-4 text-xs font-mono border border-dashed border-base-300 dark:border-base-400"
					>
			...
			<div class="flex flex-col gap-2">
				<div class="flex items-center justify-between">
					...
					{#if details && !details.payloadRedacted && hasBody(details.response?.body)}
						<CopyButton
							text={typeof details.response?.body === 'string'
				...
				{:else if details && hasBody(details.response?.body)}
					...
					>
						<JsonPreview
							...
							ariaLabel="Response Tool Result"
							maximizable
						/>
					</div>
				{:else}
				...
				<div class="flex flex-col gap-2 p-4 rounded-xl bg-error/10 border border-error/20">
					...
					<p class="text-xs font-mono text-error font-medium leading-relaxed">
						{auditLog.outcome.error ||
							auditLog.outcome.reason ||
							'Yêu cầu bị từ chối do chính sách phân quyền'}
					</p>
				</div>
			{/if}

			<div class="divider my-0 text-xs font-semibold text-muted-content uppercase tracking-wider">
				Thông tin chi tiết (Metadata)
			</div>

			<!-- Event Meta Table -->
			<div
				class="rounded-xl border border-base-300 dark:border-base-400 bg-base-100 dark:bg-base-300/30 p-4 flex flex-col gap-2.5 text-xs"
			>
				{@render metaRow('Request ID', details?.trace?.requestID || auditLog.id)}
				{@render metaRow('Session ID', details?.trace?.sessionID || '—')}
				{@render metaRow(
					'Agent / Client',
					[details?.agent?.provider, details?.client?.name || auditLog.client]
						.filter(Boolean)
						.join(' / ') || '—'
				)}
				...
				{@render metaRow(
					'Target MCP',
					auditLog.target.parent?.name || auditLog.target.parent?.id || auditLog.target.name || '—'
				)}
				{@render metaRow('Target Function', auditLog.target.name || auditLog.action.name || '—')}
				{@render metaRow(
					'HTTP Code / Reason',
					[auditLog.outcome.httpStatus, auditLog.outcome.reason].filter(Boolean).join(' / ') || '—'
				)}
				{@render metaRow(
					'Ghi nhận lúc',
			...
			<div
				class="flex flex-col rounded-xl border border-base-300 dark:border-base-400 bg-base-100 dark:bg-base-300/30 overflow-hidden"
			>
				<button
				...
				{#if showForensics}
					<div
						class="p-4 pt-0 border-t border-base-200 dark:border-base-400/40 flex flex-col gap-4 text-xs"
						in:slide={{ axis: 'y' }}
					>
						...
						{#if details?.client || details?.agent}
							{@const agentCli = [details?.agent?.cliName, details?.agent?.cliVersion]
								.filter(Boolean)
								.join(' ')}
							{@const agentModel = [details?.agent?.model, details?.agent?.modelID]
								.filter(Boolean)
								.join(' / ')}
							<div class="flex flex-col gap-1.5 mt-2">
								<span class="font-semibold text-slate-700 dark:text-slate-300"
									>Client & Agent Context:</span
								>
								<div class="bg-base-200 dark:bg-base-400/30 p-3 rounded-xl flex flex-col gap-1">
						...
						{#if details?.environment}
							<div class="flex flex-col gap-1.5 mt-2">
								<span class="font-semibold text-slate-700 dark:text-slate-300"
									>Môi trường Thực thi (Environment):</span
								>
								<div class="bg-base-200 dark:bg-base-400/30 p-3 rounded-xl flex flex-col gap-1">
						...
						{#if details && !details.payloadRedacted}
							{#if hasBody(details.request?.mutatedBody)}
								<div class="flex flex-col gap-1.5 mt-3">
									<span class="font-semibold text-slate-700 dark:text-slate-300"
										>Mutated Request Body:</span
									>
									<JsonPreview
										value={details.request?.mutatedBody}
										ariaLabel="Mutated Request Body"
										maximizable
									/>
								</div>
							{/if}
							{#if hasBody(details.response?.originalBody)}
								<div class="flex flex-col gap-1.5 mt-2">
									<span class="font-semibold text-slate-700 dark:text-slate-300"
										>Original Response Body:</span
									>
									<JsonPreview
										value={details.response?.originalBody}
										ariaLabel="Original Response Body"
										maximizable
									/>
								</div>
							{/if}
						...
						{#if details?.webhookStatuses?.length}
							<div class="flex flex-col gap-1.5 mt-2">
								<span class="font-semibold text-slate-700 dark:text-slate-300"
									>Webhook Statuses:</span
								>
								<JsonPreview
									value={details.webhookStatuses}
									ariaLabel="Webhook Statuses"
									maximizable
								/>
							</div>
						{/if}
...
{#snippet chip(label: string, value: string | undefined | null, customClass?: string)}
	{#if value}
		<div
			class={twMerge(
				'rounded-full px-3 py-1 text-[11px] font-medium bg-base-300 dark:bg-base-400/80 text-base-content',
				customClass
			)}
		>
...
{#snippet metaRow(label: string, value: string | number | undefined | null)}
	{#if value !== undefined && value !== null && value !== ''}
		<div
			class="grid grid-cols-12 gap-2 py-0.5 border-b border-base-200 dark:border-base-400/30 last:border-0"
		>
