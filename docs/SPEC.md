...
- GitHub MCP (#31) pilot song song với connector GitHub REST; ghim endpoint `https://api.githubcopilot.com/mcp/` qua PAT Bearer; bọc công cụ ghi `issue_write` thành các tool riêng biệt (`github_issue_create`, `github_issue_close`, `github_issue_label`) để owner phân quyền hẹp; không tự công bố tool ghi mới khi chưa có owner duyệt.
- Quét đủ tool & kiểm tra quyền (#32): MCP tools/list phân trang không giới hạn tùy tiện; khi đồng bộ (`connector_sync`), Gen-hub chủ động kiểm tra quyền token với từng tool (đối chiếu scope GitHub X-OAuth-Scopes, Slack auth.test, Google Drive readonly vs write...), hiển thị 1 trong 3 trạng thái: khả dụng / thiếu quyền (kèm scope cần) / không xác định được; phân biệt rõ lỗi thiếu quyền (HTTP 403 / missing_scope) với lỗi mạng hoặc sai token.
- Migration ID cũ (#50): Chuyển đổi ID cũ (không tiền tố) sang định dạng chuẩn (#40) chạy thủ công qua lệnh CLI riêng biệt (`gen-hub migrate-ids` hoặc `node server/migrate-ids.mjs`), trong một transaction `store.tx()`. Cập nhật đồng thời primary key, `agent.permissions` (`mid:tool` và `vault:sid`), `token.agent`/`code.agent` và các liên kết liên quan mà không làm đổi token secret/digest. Audit log lịch sử giữ nguyên không sửa, ghi thêm 1 bản ghi `system.id_migration` ghi nhận bảng ánh xạ cũ→mới.
