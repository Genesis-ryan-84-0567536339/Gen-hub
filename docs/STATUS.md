...
|---|---|---|---|---|
| **Gitea Storage & Lifecycle** | `implemented`<br>`configured`<br>`verified-live` | Tích hợp Gitea rootless 1.27.3 bắt buộc trong mọi bản cài; Caddy reverse proxy route `/gitea/`; 2 named volumes độc lập `gen-hub-<id>-gitea-data` & `config`; các lệnh vòng đời `status`, `doctor`, `backup`, `rollback`, `gitea-enable`. | PR #56, #57<br>Commit `69a7a12`<br>[GITEA_OPERATIONS.md](GITEA_OPERATIONS.md) | Dùng Git HTTPS; SSH, Actions và package registry chưa mở trong bản hiện tại. |
