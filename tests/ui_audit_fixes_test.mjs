...
test('Audit Fixes UI: Settings defaults normalization, KPI agent-only calls, and Admin copy endpoint', async t => {
  ...
  // 2. Test KPI "Lượt gọi hôm nay" on Overview (Audit Fix 2 - 5.2)
  ...
  const selectedRetention = await page.locator('form#settings select[name="retention"]').inputValue();
  assert.equal(selectedRetention, '30', 'Lưu nhật ký phải chọn 30 ngày theo mặc định chuẩn hóa (không phải 7 ngày)');
